...
在`consumeEvents`方法中，通过`mInputConsumer`读取输入事件`InputEvent`，然后根据事件类型，调用到`InputEventReceiver`不同的处理事件的方法。我们这里主要关注键盘事件（`AINPUT_EVENT_TYPE_KEY`）和触摸事件（`AINPUT_EVENT_TYPE_MOTION`）。当为这两个事件时，回到用jni方法
