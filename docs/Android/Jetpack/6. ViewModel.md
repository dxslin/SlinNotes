...
> 1. 当Activity因配置（Configuration）改变重新创建时，`ActivityThread`调用`OnDestroy()`前会执行Activity的`retainNonConfigurationInstances()`方法，将数据保存到`ActivityClientRecord.lastNonConfigurationInstances`中，待重新创建Activity时，会调用`Activity.attach()`方法，将数据重新传进新的Activity。（具体可以查看文章[ConfigurationChange执行流程](todo)）；  
